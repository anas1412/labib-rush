...
// whole life from that spawn data (see material.ts). Per frame the CPU only advances a clock and
// uploads the ring slots written since the last frame: no allocations, no per-particle work.
//
...
const STRIDE = 32; // floats per particle (8 × vec4)
...
const BIN_RIM = 1.05; // props/bin.ts opening height
const HEAD = 1.2; // dizzy stars orbit about Labib's head height
...
/** Mutable particle spec reused for every spawn. Field order = the GPU layout. */
class Spec {
  ...
  phys(drag: number, grav: number, wind: number, mode: number): this { const f = this.f; f[20] = drag; f[21] = grav; f[22] = wind; f[23] = mode; return this; }
...
export function createFx(ctx: BuildContext): Fx {
  ...
  const countScale = QUALITY_COUNT[ctx.quality];
  const S = new Spec();
  ...
  /** Short glow flash; `ground` softens where it meets the floor. */
  const flash = (x: number, y: number, z: number, ground: number, hex: number, k: number, s0: number, s1: number, life: number, delay = 0) => {
    setCol(col, hex);
    pools.glow.push(S.reset().pos(x, y, z, time + delay).vel(0, 0, 0, life).size(s0, s1).c0(col, k, 1).c1(col, k * 0.4, 1).phys(0, 0, 0, MOTION_DRAG + BILLBOARD)
  ...
  const dust = (p: Vector3, n: number, sc: number, hex: number, d: Vector3 | undefined) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      pools.soft.push(S.reset().pos(p.x + Math.cos(a) * r, p.y + (0.12 + rnd(0, 0.1)) * sc, p.z + Math.sin(a) * r, time)
        ...
        .size(0.22 * sc, rnd(0.6, 0.9) * sc, undefined, rnd(-0.7, 0.7))
        .c0(col, 1, rnd(0.6, 0.78)).c1(col, 1.05, 0).phys(3.2, -0.3, 0.5, MOTION_DRAG + BILLBOARD).ground(p.y).extra(0, 0, 0, SHAPE.puff));
    }
  };
  ...
  const EMIT: Record<FxKind, (p: Vector3, n: number, sc: number, hex: number | undefined, d: Vector3 | undefined) => void> = {
    sparkle(p, n, sc, hex = 0xffdf8f) {
      flash(p.x, p.y + 0.2 * sc, p.z, p.y, hex, 2.2, 0.3 * sc, 0.85 * sc, 0.18);
      setCol(col, hex);
      for (let i = 0; i < n; i++) {
    ...
    dust(p, n, sc, hex = 0xe6dac4, d) { dust(p, n, sc, hex, d); },
    ...
    spill(p, n, sc, _hex, d) {
      ...
      dust(p, 5, 1.2 * sc, 0xe6dac4, undefined);
      setCol(col, 0xfff4d6);
      for (let i = 0; i < 5; i++) {
    ...
    hit(p, n, sc) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        pools.glow.push(S.reset().pos(p.x, y, p.z, time).vel(Math.cos(a) * sp, rnd(0.2, 0.9) * sp, Math.sin(a) * sp, rnd(0.45, 0.7))
          .size(0.26 * sc, 0.1 * sc, undefined, rnd(4, 8) * sgn()).c0(col, 1.8, 1).c1(col2, 1.2, 1).phys(5, 3, 0, MOTION_DRAG + BILLBOARD).extra(0, 0, 0, SHAPE.star));
      }
      for (let i = 0; i < 4; i++) { // dizzy stars circling the head during the stun
        pools.glow.push(S.reset().pos(p.x, p.y + HEAD * sc, p.z, time + 0.08).vel((i / 4) * 6.283, 0, 0, 1.05)
          .size(0.15 * sc, 0.13 * sc, undefined, 3).c0(col, 1.6, 1).c1(col, 1.4, 1).phys(0, 0, 0, MOTION_SWIRL + BILLBOARD)
          .axis(0.34 * sc, 7, 0.34 * sc).extra(0, 0, 0, SHAPE.star));
      }
      dust(p, 4, sc, 0xe6dac4, undefined);
    },

    deposit(p, n, sc, hex = 0x4dff94) {
      const rim = p.y + BIN_RIM;
      setCol(col, hex);
      const a0 = Math.random() * 6.283;
      for (let i = 0; i < n; i++) {
        pools.glow.push(S.reset().pos(p.x, rim + rnd(0.35, 0.7) * sc, p.z, time).vel(a0 + (i / n) * 6.283, -rnd(0.9, 1.4), 0, rnd(0.5, 0.7))
          .size(0.18 * sc, 0.06 * sc, undefined, rnd(-2, 2)).c0(col, 2.4, 1).c1(white, 1.5, 1)
          .phys(0, 0.8, 0, MOTION_SWIRL + (i % 2 ? STREAK : BILLBOARD)) // half drawn as arcs tracing the spiral
          .axis(rnd(0.5, 0.75) * sc, 9, 0.04).extra(0, 0, i % 2 ? 0.35 * sc : 0, i % 2 ? SHAPE.streak : SHAPE.flare));
      }
      flash(p.x, rim, p.z, NO_GROUND, hex, 2.4, 0.25 * sc, 1 * sc, 0.25, 0.5);
      for (let i = 0; i < 8; i++) {
        const a = Math.random() * 6.283, sp = rnd(0, 0.6);
        pools.glow.push(S.reset().pos(p.x, rim, p.z, time + 0.5).vel(Math.cos(a) * sp, rnd(2, 3.2) * sc, Math.sin(a) * sp, rnd(0.4, 0.6))
          .size(0.08 * sc, 0.015).c0(col, 2.2, 1).c1(col, 1.1, 1).phys(2, 6, 0, MOTION_DRAG + BILLBOARD).extra(0, 0, 0, SHAPE.glow));
      }
    },
