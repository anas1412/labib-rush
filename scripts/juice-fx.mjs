...
//     [--view=gc|backlit]  gc = the game's follow camera (5.5 m behind, pivot 1.3 m), backlit = the same
//                          distance looking into the low sun      [--extra=&shade=0]  more URL params
//     [--tag=name]         output file prefix (default fx)
...
// sun (toward) is WSW: (−0.888, 0.329, 0.321) → looking into it from 5.5 m means standing east-north-east
const CAMS = { gc: { cam: '0,2.4,5.5', target: '0,1.0,0' }, backlit: { cam: '5.2,2.4,-1.9', target: '0,1.1,0' } };
const tag = opt.tag || (opt.view ? `fx-${opt.view}` : 'fx');
...
for (const k of kinds) {
  ...
  for (const t of times) {
    const c = CAMS[opt.view] ?? v;
    const url = `http://127.0.0.1:5180/dev/fx.html?kind=${k}&freeze=${t}&cam=${c.cam}&target=${c.target}&engine=${opt.engine ?? 1}&q=${opt.q ?? 'high'}${opt.extra ?? ''}`;
    await page.goto(url, { waitUntil: 'load' });
    await page.waitForFunction(() => window.__ready === true, null, { timeout: 90000 });
    await page.waitForTimeout(Number(opt.wait ?? 900));
    const f = `${OUT}/${tag}-${k}-${t}.png`;
...
await browser.close();
execFileSync('magick', ['montage', ...files, '-tile', '3x', '-geometry', `${Math.round(W / 2.5)}x${Math.round(H / 2.5)}+3+3`, '-background', '#111', `${OUT}/${tag}-sheet.png`]);
